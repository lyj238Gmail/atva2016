...
%Among all the work, our work is the most automatic, and the  set of auxiliary invariants is the most complete. Both invariants are searched automatically and the formal proof is generated automatically. Besides, we make efforts to illustrate the semantical intuition behinds these invariants.
...
Inductive invariants are provided by \bedt{hand}, and the theorem prover must be manually guided to perform the induction proof to prove the correspondence between the aggregated protocol  and the original one. %This work is really pioneering because later research on FLASH protocol must also rely on the version of FLASH protocol in \cite{}. Especially, the auxiliary
...
\end{itemize}%There are ? lines in the causal relation table.%=========================================%\subsection{ Parameterized Proof Script}%=========================================
%\vspace{-2cm}
...
\item[2] Definitions of formally parameterized rules%,  which can be directly transformed from the  Murphi rules of FLASH protocol. There are 62 rules.
