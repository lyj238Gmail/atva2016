...
%=========================================
Our case study on FLASH is a  typically successful application of {\sf paraVerifier} which combines   
model checking and theorem proving. The
consistency lemma based on the induction approach, is the
core of our work, which gives the heuristics to guide the tool
 to search invariants. By model checking candidate invariants in , by generalizing , we theorem proving
 
 
 In this sense, we realize the 
 
 Compared with previous work, the invariants we construct is the most abundant. From this aspect, our understanding is . In fact , we can axioma semantics of . Intuitively we We have control variables and .It is also interesting to relate our approach to previous work?
 
 Instead of ``invisible invariants" in previous work
